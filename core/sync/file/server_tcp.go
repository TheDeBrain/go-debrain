...
func HandleGetFileResponseTCP(conn net.Conn) error {
	// get file protocol
	...
	if len(m) > 0 {
		...
		res, _ := protocols.RESNew(conn.LocalAddr().String(), string(sys.TSysNew().SyncPortTCP),
...
func HandleUploadSyncReqTCP(conn net.Conn) error {
	// read in sync net
...
func HandleFileBlockServerBroadCastSyncTCP(conn net.Conn) error {
